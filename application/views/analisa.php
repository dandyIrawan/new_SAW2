...
<div class="panel panel-info">
  ...
  <div class="panel-body">
  	<table class="table table-bordered">
  		...
  		<tbody>
  		<?php 
  		$kr1 = $this->db->query("SELECT * FROM siswa ");
  		foreach ($kr1->result() as $a) {
  		 ?>
  			<tr>
  				...
  				<?php 
  				$id = $a->id_siswa;
  				$kr2 = $this->db->query("SELECT DISTINCT kriteria.kriteria,nilai.nilai FROM nilai,kriteria WHERE nilai.id_kriteria=kriteria.id_kriteria AND nilai.id_siswa='$id'");
  				foreach ($kr2->result() as $a) {
  				 ?>
  		...
  		foreach ($kr1->result() as $a) {
  		 ?>
  			<tr>
  				...
  				<?php 
  				$id = $a->id_siswa;
  				$kr2 = $this->db->query("SELECT DISTINCT kriteria.kriteria,normalisasi.nilai_normalisasi FROM normalisasi,kriteria WHERE normalisasi.id_kriteria=kriteria.id_kriteria AND normalisasi.id_siswa='$id'");
  				foreach ($kr2->result() as $a) {
  				 ?>
  		...
  		foreach ($kr1->result() as $a) {
  		 ?>
  			<tr>
  				...
  				$id = $a->id_siswa;
  				$n_a = $a->nilai_analisa;
  				$kr2 = $this->db->query("SELECT DISTINCT kriteria.kriteria, analisa.nilai FROM analisa,kriteria WHERE analisa.id_kriteria=kriteria.id_kriteria AND analisa.id_siswa='$id'");
  				foreach ($kr2->result() as $a) {
  				 ?>
  				<td><?php echo $a->nilai; ?></td>
  				<?php } ?>

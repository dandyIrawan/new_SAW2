...
class Nilai extends CI_Controller
{
    ...
    {
        $pg = $this->db->query("SELECT DISTINCT nilai.id_siswa, nilai.id_kriteria FROM nilai,kriteria,siswa WHERE kriteria.id_kriteria=nilai.id_kriteria AND siswa.id_siswa=nilai.id_siswa");
        foreach ($pg->result() as $a1) {
            $idker = $a1->id_kriteria;
            $idpeg = $a1->id_siswa;
            $max = $this->db->query("SELECT kriteria.kriteria, MAX(nilai.nilai) as hasilmax FROM nilai,kriteria,siswa WHERE kriteria.id_kriteria=nilai.id_kriteria AND kriteria.id_kriteria='$idker' ")->row();
            $nil = $this->db->query("SELECT nilai FROM nilai WHERE id_kriteria='$idker' AND id_siswa='$idpeg'")->row();
            // echo $a1->id_siswa.' - '. number_format($nil->nilai/$max->hasilmax,2).' - '.$a1->id_kriteria.'<br>';

            $data = array(
                'id_siswa' => $a1->id_siswa,
            ...
            $cek = $this->db->query("SELECT * FROM normalisasi WHERE id_siswa='$a1->id_siswa' and id_kriteria='$a1->id_kriteria' and nilai_normalisasi='$nimax'");
    ...
    public function proses_analisa() 
    {
        ...
        foreach ($pg->result() as $a1) {
            ...
            $nil = $this->db->query("SELECT nilai_normalisasi FROM normalisasi WHERE id_kriteria='$idker' AND id_siswa='$idpeg'")->row();
            echo $a1->id_siswa.' - '. number_format($nil->nilai_normalisasi*$bobot->bobot,2).' - '.$a1->id_kriteria.'<br>';
            ...
            $cek = $this->db->query("SELECT * FROM analisa WHERE id_siswa='$a1->id_siswa' and id_kriteria='$a1->id_kriteria' and nilai='$nimax'");
            if ($cek->num_rows() == null) {
        ...
        $kr1 = $this->db->query("SELECT * FROM siswa order by nilai_analisa desc");
        foreach ($kr1->result() as $a) {
            ...
            foreach ($kr3->result() as $row) {
                ...
                $kr4 = $this->db->query("SELECT nilai FROM analisa WHERE id_siswa='$id' and id_kriteria='$id_kriteria'")->row();
                // echo $kr4->nilai.'<br>';
                $sum = $sum + $kr4->nilai;
